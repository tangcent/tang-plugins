...
import com.itangcent.idea.plugin.util.invokeMethod
import com.itangcent.idea.plugin.util.reduceSafely
    ...
    fun getTypeObject(psiType: PsiType?, context: PsiElement, option: Int): Any? {
        ...
        when {
            ...
            isCollection(castTo) -> {   //list type
                ...
                when {
                    classTypeName != null && isNormalType(classTypeName) -> getDefaultValue(classTypeName)?.let { list.add(it) }
...
//                kv[name] = getTypeObject(type, psiClass.context!!, option)
                    ...
                    if (fieldType is PsiPrimitiveType) {       //primitive Type
                        ...
                        when {
                            ...
                            type is PsiArrayType -> {   //array type
                                ...
                                when {
                                    deepType is PsiPrimitiveType -> list.add(PsiTypesUtil.getDefaultValue(deepType))
                                    isNormalType(deepType.canonicalText) -> getDefaultValue(deepType.canonicalText)?.let { list.add(it) }
            ...
            when {
                ...
                type is PsiArrayType -> {   //array type
                    ...
                    when {
                        deepType is PsiPrimitiveType -> list.add(PsiTypesUtil.getDefaultValue(deepType))
                        isNormalType(deepType.canonicalText) -> getDefaultValue(deepType.canonicalText)?.let { list.add(it) }
                        else -> getTypeObject(tmTypeHelper!!.ensureType(deepType, clsWithParam.typeParams), context, option)?.let { list.add(it) }
                    }
                    return copy(list)
                }
                ...
                isCollection(type) -> {   //list type
                    ...
                    when {
                        classTypeName != null && isNormalType(classTypeName) -> getDefaultValue(classTypeName)?.let { list.add(it) }
                        iterableType != null -> getTypeObject(tmTypeHelper!!.ensureType(iterableType, clsWithParam.typeParams), context, option)?.let { list.add(it) }
                    }
                ...
                isMap(type) -> {
                    ...
                    if (defaultKey == null) {
                        if (keyType != null) {
                            defaultKey = getTypeObject(tmTypeHelper!!.ensureType(keyType, clsWithParam.typeParams), context, option)
                        } else {
                            defaultKey = ""
                        }
                    }
                    ...
                    if (defaultValue == null) {
                        if (valueType != null) {
                            defaultValue = getTypeObject(tmTypeHelper!!.ensureType(valueType, clsWithParam.typeParams), context, option)
                        } else {
                            defaultValue = null
                        }
                    }
                ...
                if (!kv.contains(fieldName)) {
                    ...
                    when {
                        ...
                        isCollection(type) -> {   //list type
                            ...
                            val classTypeName: String? = iterableClass?.qualifiedName
                            when {
                                classTypeName != null && isNormalType(classTypeName) -> list.add(getDefaultValue(classTypeName)!!)
                                iterableClass == psiClass -> list.add(Collections.emptyMap<Any, Any>())
                                iterableType != null -> getTypeObject(tmTypeHelper!!.ensureType(iterableType, clsWithParam.typeParams), context, option)?.let { list.add(it) }
                            }
                            kv[fieldName] = list
                        ...
                        isMap(type) -> {   //list type
                            ...
                            var defaultKey: Any? = null
                            if (keyType != null) defaultKey = getTypeObject(tmTypeHelper!!.ensureType(keyType, clsWithParam.typeParams), context, option)
                            if (defaultKey == null) defaultKey = ""

                            var defaultValue: Any? = null
                            if (valueType != null) defaultValue = getTypeObject(tmTypeHelper!!.ensureType(valueType, clsWithParam.typeParams), context, option)
                            if (defaultValue == null) defaultValue = null
    ...
    private fun foreachField(psiClass: PsiClass, option: Int, handle: (name: String, type: PsiType, fieldOrMethod: PsiElement) -> Unit) {
    ...
    @Suppress("UNCHECKED_CAST")
    fun resolveEnumOrStatic(classNameWithProperty: String, psiMember: PsiMember, defaultPropertyName: String): ArrayList<HashMap<String, Any?>>? {
        val options: ArrayList<HashMap<String, Any?>> = ArrayList()
        ...
        if (cls.isEnum) {
            ...
            for (enumConstant in enumConstants) {
                ...
                options.add(KV.create<String, Any?>()
                        .set("value", mappedVal)
                        .set("desc", desc)
                )
    ...
    fun resolveClass(className: String, psiMember: PsiMember): PsiClass? {
        return when {
            ...
            else -> getContainingClass(psiMember)?.let { resolveClassFromImport(it, className) } ?: tmTypeHelper!!.findClass(className, psiMember)
        }
    }
...
//        val multipartFileInstance = "file"
        ...
        var jvmStaticFinalFieldModifiers: Set<JvmModifier> = HashSet(Arrays.asList(JvmModifier.STATIC, JvmModifier.FINAL))
