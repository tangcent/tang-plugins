...
import com.itangcent.idea.plugin.util.changePropertyValue
import com.itangcent.idea.plugin.util.getPropertyValue
...
//            val evalFun: () -> Unit = { pool { evalFun(exp) } }
    ...
    class AutoBind2<T, P1, P2> : AutoBind<T, (P1?, P2?) -> T?, AutoBind2<T, P1, P2>> {
        ...
        override fun evalFun(exp: (P1?, P2?) -> T?): () -> Unit {
            return {
                core.computer.value(core.property, exp(this.core.params.get(0).get() as P1, this.core.params.get(1).get() as P2))
            }
        }
    }

    class AutoBind3<T, P1, P2, P3>(core: AutoBindData<T>) : AutoBind<T, (P1?, P2?, P3?) -> T?, AutoBind3<T, P1, P2, P3>>(core) {

        @Suppress("UNCHECKED_CAST")
        override fun evalFun(exp: (P1?, P2?, P3?) -> T?): () -> Unit {
            return {
                core.computer.value(core.property, exp(this.core.params.get(0).get() as P1,
...
class BeanPropertyWrap<T> : ASetter<T?>, AGetter<T?>, HasProperty<T> {
    ...
    constructor(targetGetter: () -> Any?, propertySetter: (Any, T?) -> Unit, propertyGetter: (Any) -> T?, aProperty: AProperty) {
