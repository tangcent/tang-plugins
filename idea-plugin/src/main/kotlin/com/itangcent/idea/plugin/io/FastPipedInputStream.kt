package com.itangcent.idea.plugin.io
...
class FastPipedInputStream : InputStream {
    ...
    @JvmOverloads constructor(src: FastPipedOutputStream, pipeSize: Int = DEFAULT_PIPE_SIZE) {
        initPipe(pipeSize)
        connect(src)
    }
     ...
     * It must be [connected][com.itangcent.idea.plugin.io.FastPipedOutputStream.connect] to a
     ...
     * It must be [ connected][com.itangcent.idea.plugin.io.FastPipedOutputStream.connect] to a `PipedOutputStream` before being used.
     *
     * @param pipeSize the size of the pipe's buffer.
     * @throws IllegalArgumentException if `pipeSize <= 0`.
     * @since 1.6
