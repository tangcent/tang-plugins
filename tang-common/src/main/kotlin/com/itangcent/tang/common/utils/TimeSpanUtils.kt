...
object TimeSpanUtils {
    ...
    private val units = arrayOf(TimeUnit.NANOSECONDS, TimeUnit.MICROSECONDS, TimeUnit.MILLISECONDS, TimeUnit.SECONDS, TimeUnit.MINUTES, TimeUnit.HOURS, TimeUnit.DAYS)
    ...
    fun parse(timeSpan: String): Long? {
        ...
        match(timeSpan, BiConsumer { duration, unit -> holder.updateData(Function { span -> span!! + convert(duration, unit, TimeUnit.MILLISECONDS).toLong() }) })
    ...
    fun precisionParse(timeSpan: String, timeUnit: TimeUnit = TimeUnit.MILLISECONDS): Double? {
        ...
        match(timeSpan, BiConsumer { duration, unit -> holder.updateData(Function { span -> span!! + convert(duration, unit, timeUnit) }) })
        return holder.getData()
    }
